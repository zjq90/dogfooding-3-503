<template>
  <div class="my-reserves">
    <el-card>
      ...
      <el-empty v-if="reserveList.length === 0" description="暂无预订记录" />
    </el-card>
  </div>
</template>
...
import { getReservesByUser, cancelReserve } from '@/api/reserve'
...
const loadData = async () => {
  ...
  try {
    ...
    if (userName) {
      const res = await getReservesByUser(userName)
